    ...
    if ($adminOrder) {
      ...
      foreach ($_SESSION['cart'] as $item) {
        ...
        if (!empty($item['variantId']) && $item['id'] == $product['id']) {
          ...
          $priceWithDiscoutn = $cart->applyDiscountSystem($product['price']); 
          ...
          $discountSystem = $priceWithDiscoutn['discounts']; 
          $promocode = $priceWithDiscoutn['discounts'] !='' ? $priceWithDiscoutn['promo'] : $_SESSION['couponCode'];
          //По ходу формируем массив запросов на обновление количества товаров
  ...
  public function getParamArray($pay, $orderId, $summ) {
    ...
    if (!empty($jsonPaymentArray)) {
      ...
      if(15 == $pay){
        ...
        $payment = 'amt='.$summ.'&ccy=UAH&details=заказ на '.SITE.'&ext_details='.$order['orderNumber'].'&pay_way=privat24&order='.$orderId.'&merchant='.$paramArray[0]['value'];

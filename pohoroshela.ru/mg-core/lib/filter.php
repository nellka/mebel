...
class Filter {
 ...
 /**
  ...
  public function getHtmlPropertyFilter() { 
    ...
    $_REQUEST['category_id'] = $_REQUEST['category_id']?$_REQUEST['category_id']:$_REQUEST['cat_id'];
    $property = Storage::get(md5('filterProperty'.$_REQUEST['category_id']));   
    
    if($property == null){  
      $property = $this->getPropertyData();
      Storage::save(md5('filterProperty'.$_REQUEST['category_id']),$property);
    }
    ...
    // приводим к одному виду все значения характеристик в выбраных фильтрах заменяем 
    ...
    foreach ($property as $idProp => $prop) {
      ...
      if(!empty($values)){
        ...
        #ДОБАВЛЕНО новый тип вывода характеристики (слайдер)
          ...
          foreach ($values as $value) {          
            ...
            if(!empty($value)){
              ...
              if(!empty($this->accessValues[$idProp]) && in_array($value, $this->accessValues[$idProp])){     
                ...
                $html .= ' <li style="'.$style.'"><label><input  type="checkbox" name="prop['.$idProp.'][]" value="'.$value.'" '.$checked.'  class="mg-filter-prop-checkbox"/>'.$value.'</label>'.$viewAll.'</li>';
              }elseif(empty($this->accessValues[$idProp])){  
                $value = htmlspecialchars($value);
                $html .= ' <li style="'.$style.'"><label><input type="checkbox" name="prop['.$idProp.'][]" value="'.$value.'" '.$checked.'  class="mg-filter-prop-checkbox"/>'.$value.'</label>'.$viewAll.'</li>';
              }else{
                $value = htmlspecialchars($value);
                $html .= ' <li style="'.$style.'"><label class="disabled-prop"><input disabled type="checkbox" name="prop['.$idProp.'][]" value="'.$value.'" '.$checked.'  class="mg-filter-prop-checkbox"/>'.$value.'</label>'.$viewAll.'</li>';
               }
              $i++; 
            }
          }
        ...
        if($prop['type_filter']=='slider'){
          $i = 0;
          $min = (float)$values[1]; // Максимальное значение (назначим 1 значение самым маленьким)
          $max = (float)$values[1]; // Минимальное значение  (назначим 1 значение самым большим) 
          ...
          // Если рассмотреных значений меньше 2, нет смысла выводить слайдер    
            $html .= '
            <li>
  ...
  private function getPropertyData(){
    ...
    $sql = "
      ...
      WHERE cp.category_id IN (".DB::quote($categoryIds,true).") and pp.filter = 1

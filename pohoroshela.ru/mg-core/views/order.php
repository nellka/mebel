      ...
      <div class="payment-option">
        ...
        <form action="<?php echo SITE ?>/order?creation=1" method="post">
          ...
          <div class="delivery-vs-payment">
            <?php if (''!=$data['delivery']): ?>
              ...
              <ul class="delivery-details-list">
                ...
                <?php foreach ($data['delivery'] as $delivery): ?>
                  <?php $summDelivery = (($delivery['checked'] && $delivery['cost'] > 0)? $delivery['cost'] : $summDelivery) ?>
            ...
            <?php endif; ?>
              <div class="delivery-date" style="display:none; margin-bottom: 5px;">
            ...
            <div class="summ-info" style="margin-bottom: 5px;">
              <span class="delivery-text">Сумма заказа:</span>
              <span class="order-summ"><?php echo $data['summOrder']?></span>
              <span class="delivery-summ"><?php if ($summDelivery != '') { ?>
                  + доставка: <span class="order-summ"><?php echo $summDelivery?> </span> 
              <?php } ?></span>
            </div>
          </div>
           <?php if ($data['captcha']) { ?>   
  ...
  case 2:
    ...
    <?php else: ?>
      <div class="payment-form-block">
        ...
        <?php if ($data['linkToStatus']) :?>
        <br>Следить за статусом заказа Вы можете по ссылке <br> <a href="<?php echo SITE.'/order?hash='.$data['linkToStatus']?>">
          <?php echo SITE.'/order?hash='.$data['linkToStatus']?></a>
        <?php endif;?>
        <hr>

    ...
    foreach($groups as $key=>$group){
        ...
        foreach($group as $optionName){
            ...
            if($option['option'] == 'cacheCssJs'){
              ...
              $input = '<input type="checkbox" class="option minify-css-and-js" name="'.$option['option'].'" value="'.$option['value'].'" '.($option['value']=='true'?'checked=checked':'').'/>';
            }
            ...
            $propertyHtml .=  "<li><span class='property-name ".$textUp."'>".$lang[$alias]."</span>".$input."<a href='javascript:void(0);' class='tool-tip-top desc-property' title='".$lang['DESC_'.$option['name']]."' >?</a></li>";
        }
        $propertyHtml .=  "</ul></div>";
    }
    ?>

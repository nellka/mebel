...
Apache HTTP Server (recommended)
    ...
    * It is recommended that AllowOverride be enabled in httpd.config for the
      directory where your guestbook is installed.
...
1) Make a backup copy of your config.php and strings.php files, the contents 
   of your "data" folder, and any files in the "template" directory that you 
   have customized.
...
the "themes" folder is not recommended, as this may break the script and/or 
make it difficult to upgrade to newer versions of DRBGuestbook in the future.
...
DRBGuestbook is provided "as is", without warranty of any kind, either 
...
If you want to use DRBGuestbook, but do not want to display the credit link at 
the bottom of each page, you must purchase a Credit Link Removal License.  
Please visit the following URL for details:

http://www.dbscripts.net/removelink/

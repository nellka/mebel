...
//		$product_data['sql']=$sql;
	...
	public function getProductRelated($product_id) {
		...
		$product_data = array();
        $sql_old = "SELECT * FROM " . DB_PREFIX . "product_related pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.related_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pr.product_id = '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= '" . $this->NOW . "' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
		//echo $sql;
		$sql = "(SELECT * FROM " . DB_PREFIX . "product_related pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.related_id = p.product_id) 
		                       LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) 
		                       LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON ( p.product_id = p2c.product_id ) 
		                       WHERE main_category =1 AND category_id <>276 AND 
		                       pr.product_id = '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= '" . $this->NOW . "' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'
		              ) UNION (SELECT * FROM " . DB_PREFIX . "product_related pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.related_id = p.product_id) 
		                       LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) 
		                       LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON ( p.product_id = p2c.product_id ) 
		                       WHERE main_category =1 AND category_id =276 AND 
		                       pr.product_id = '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= '" . $this->NOW . "' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'
...
AND pr.product_id =  '3045'
AND p.status =  '1'
AND p.date_available <=  '2015-05-06 16:39:00'
AND p2s.store_id =  '0'
AND category_id =276
